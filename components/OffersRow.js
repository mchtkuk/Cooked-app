...
const OffersRow = ({ title, id, description }) => {
  const [offeredRestaurants, setOfferedRestaurants] = useState([]);

  const getOfferedRestaurants = (endpoint, setData) => {
    axios
  ...
  useEffect(() => {
    getOfferedRestaurants("offers", setOfferedRestaurants);
  }, []);
  ...
  return (
    <View>
      ...
      <ScrollView
        ...
        className="pt-4"
      >
        {offeredRestaurants.map((restaurants) => (
          <RestaurantCard
            key={restaurants.id}
            id={restaurants.id}
